...
//from creepy salhack 
public class AutoDupe2 extends Module {
	...
	Setting delay = create("Delay", "AutoDupeIStackDelay", 1, 0, 10);
	Setting shulker_only = create("Shulker Only", "AutoDupeIStackShulkerOnly", true);
    Setting hit_ground = create("Hit Ground", "AutoDupeIStackHitGround", true);
    ...
    private final TimerUtil timer = new TimerUtil(); //How long to wait
	...
	@Override
	public void update() {
        if (Keyboard.isKeyDown(Ozark.KEY_GUI_ESCAPE)) //toggle on escape
        {
            this.set_disable();
            return;
        }
            ...
            if (l_Entity instanceof AbstractChestHorse) {
                ...
                if (!l_entity.hasChest()) {
                    ...
                    if (l_Slot != -1 && mc.player.inventory.currentItem != l_Slot) {
                        ...
                        MessageUtil.send_client_message("No chests in hotbar, toggling...");
                        set_disable();
                        return;
