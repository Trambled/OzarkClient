...
class PlayerUtil {
    ...
    public
    void addSpeed ( final Double speed ) {
        final Double yaw = this.getMoveYaw ( );
    ...
    public
    void setTimer ( final float speed ) {
        PlayerUtil.mc.timer.tickLength = 50.0f / speed;
    }

    public
    void step ( final float height , final double[] offset , final boolean flag , final float speed ) {
        if ( flag ) {
            this.setTimer ( speed );
        }
    ...
    void onTick ( TickEvent.ClientTickEvent event ) {
        ...
        if ( didJumpThisTick && ( ! mc.player.onGround || isJumping ) ) {
            if ( didJumpThisTick && ! this.didJumpLastTick ) {
                ...
                this.percentJumpSpeedChanged = speedometerCurrentSpeed != 0.0 ? speedometerCurrentSpeed / this.lastJumpSpeed - 1.0 : - 1.0;
                this.jumpSpeedChanged = speedometerCurrentSpeed - this.lastJumpSpeed;
                this.jumpInfoStartTime = Minecraft.getSystemTime ( );
                this.lastJumpSpeed = speedometerCurrentSpeed;
