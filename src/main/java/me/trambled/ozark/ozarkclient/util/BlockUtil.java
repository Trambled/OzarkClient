...
class BlockUtil {
    ...
    public static
    void placeCrystalOnBlock ( final BlockPos pos , final EnumHand hand , boolean packet ) {
    ...
    public static
    void openBlockOffhand ( BlockPos pos ) {
        EnumFacing[] facings = EnumFacing.values ( );
    ...
    public static
    boolean placeBlockShulker ( BlockPos pos , int slot , boolean rotate , boolean rotateBack , Setting setting , EnumFacing facing ) {
        if ( isBlockEmpty ( pos ) ) {
            int old_slot = - 1;
            if ( slot != mc.player.inventory.currentItem && slot != - 1 ) {
                old_slot = mc.player.inventory.currentItem;
    ...
    public static
    boolean placeBlock ( BlockPos pos , boolean rotate , boolean rotateBack , Setting setting ) {
        if ( isBlockEmpty ( pos ) ) {
            EnumFacing[] facings = EnumFacing.values ( );
