...
public
class CustomMainMenu extends GuiScreen {
    ...
    class TextButton
        ...
        void drawButton ( Minecraft mc , int mouseX , int mouseY , float partialTicks ) {
            if ( this.visible ) {
                ...
                if ( this.hovered ) {
                    RenderUtil.drawLine ( (float) ( this.x - 1 ) - (float) RainbowUtil.get_string_width ( this.displayString ) / 2.0f , this.y + 2 + RainbowUtil.get_string_height ( ) , this.x + (float) RainbowUtil.get_string_width ( this.displayString ) / 2.0f + 1.0f , this.y + 2 + RainbowUtil.get_string_height ( ) , 1.0f , Color.WHITE.getRGB ( ) );
                }
            }
        }
