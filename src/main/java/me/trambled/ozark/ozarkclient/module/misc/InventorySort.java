...
public
class InventorySort extends Module {

    Setting confirmSort = create ( "Confirm Sort" , "ConfirmSort" , true );
    Setting chatMsg = create ( "Chat MSG" , "ChatMsgs" , true );
    Setting debugMode = create ( "Debug" , "Debug" , false );
    Setting instaSort = create ( "Insta Sort" , "InstaSort" , false );
    Setting closeAfter = create ( "Close After" , "CloseAfter" , true );
    Setting tickDelay = create ( "Delay" , "TickDelay" , 0 , 0 , 20 );
    ...
    void enable ( ) {
        ...
        if ( chatMsg.get_value ( true ) )
            MessageUtil.send_client_message ( "Config " + curConfigName + " actived" );
        // Get the inventory
    ...
    void sortInventoryAlgo ( ) {
        ...
        if ( ! openedBefore ) {
            ...
            if ( sortItems.size ( ) == 0 && ! doneBefore ) {
                ...
                // Print
                if ( chatMsg.get_value ( true ) )
                    MessageUtil.send_client_error_message ( "Inventory arleady sorted..." );
    ...
    private
    ArrayList < Integer > getInventorySort ( ) {
        // Plan to move
    ...
    private
    ArrayList < String > getInventoryCopy ( ) {
