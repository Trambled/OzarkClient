...
public class Blocker extends WurstplusHack
{
    ...
    WurstplusSetting anvilBlocker = create("Anvils", "AnvilBlocker", true);
    WurstplusSetting pistonBlocker = create("Pistons", "PistonBlocker", true);
	WurstplusSetting tickDelay = create("Delay", "TickDelay", 5, 0, 10);
	WurstplusSetting chatMsg = create("Chat", "ChatMSGS", true);
    ...
    protected void enable() {
        ...
        if (chatMsg.get_value(true)) {
            ...
            if (!output.equals("")) {
                noActive = false;
                WurstplusMessageUtil.send_client_message(output +" turned ON!");
            }else {
                noActive = true;
                disable();
            }
    ...
    protected void disable() {
        ...
        if (chatMsg.get_value(true)) {
            if (noActive) {
                WurstplusMessageUtil.send_client_error_message("Nothing is active... Blocker turned OFF!");
            }else if(noObby) {
                WurstplusMessageUtil.send_client_error_message("Obsidian not found... Blocker turned OFF!");
            }
        }
