...
public class InventorySort extends WurstplusHack {
	...
	WurstplusSetting instaSort = create("Insta Sort","InstaSort",false);
	...
	protected void enable() {
		...
		if (chatMsg.get_value(true))
			WurstplusMessageUtil.send_client_message("Config " + curConfigName + " actived");
		// Get the inventory
	...
	private void sortInventoryAlgo() {
		...
		if (!openedBefore) {
			...
			if (sortItems.size() == 0 && !doneBefore) {
				...
				// Print
				if (chatMsg.get_value(true))
					WurstplusMessageUtil.send_client_error_message("Inventory arleady sorted...");
				// If we are using instaSort, close
				if (instaSort.get_value(true)|| closeAfter.get_value(true)) {
