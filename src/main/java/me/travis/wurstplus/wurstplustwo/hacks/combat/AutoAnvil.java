...
public class AutoAnvil extends WurstplusHack
{
    ...
    WurstplusSetting tick_delay = create("Tick Delay", "Tick Delay", 5, 1, 10);
    WurstplusSetting blocks_per_tick = create("Blocks Per Tick", "BPS", 5, 1, 10);
    WurstplusSetting range = create("Range", "Range", 4, 0, 6);
    WurstplusSetting decrease = create("Decrease", "Decrease", 4, 0, 8);
    WurstplusSetting h_distance = create("H Distance", "HDistance", 7, 1, 10);
    WurstplusSetting min_h = create("Min H", "MinH", 3, 1, 10);
    WurstplusSetting fail_stop = create("Fail Stop", "FailStop", 2, 1, 10);
    WurstplusSetting anti_crystal = create("AntiCrystal", "AntiCrystal", false);
	WurstplusSetting rotate = create("Rotate", "Rotate", true);
    WurstplusSetting fast_anvil = create("FastAnvil", "FastAnvil", true);
    ...
    protected void disable() {
        ...
        if (chatMsg.get_value(true)) {
            if (noMaterials) {
                WurstplusMessageUtil.send_client_message("No Materials Detected... AutoAnvil turned OFF!");
            }
            else if (!isHole) {
                WurstplusMessageUtil.send_client_message("Enemy is not in a hole... AutoAnvil turned OFF!");
            }
            else if(!enoughSpace) {
                WurstplusMessageUtil.send_client_message("Not enough space... AutoAnvil turned OFF!");
            }
            else if(hasMoved) {
                WurstplusMessageUtil.send_client_message("Enemy moved away from the hole... AutoAnvil turned OFF!");
            }
            else if(blockUp) {
                WurstplusMessageUtil.send_client_message("Enemy head blocked.. AutoAnvil turned OFF!");
            }
            else {
                WurstplusMessageUtil.send_client_message("AutoAnvil turned OFF!");
            }
        }
	...
	public static boolean basicChecksEntity(Entity pl) {
        return pl == mc.player || WurstplusFriendUtil.isFriend(pl.getName()) || pl.isDead;
    }
