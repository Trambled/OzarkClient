...
import me.travis.wurstplus.wurstplustwo.command.commands.AutoGearCommand;
...
public class AutoGear extends WurstplusHack {
	...
	WurstplusSetting tickDelay = create("Delay","TickDelay",0, 0, 20);
	WurstplusSetting chatMsg = create("Chat MSG","ChatMsgs",true);
	WurstplusSetting confirmSort = create("Confirm Sort","ConfirmSort",true);
	WurstplusSetting invasive = create("Invasive","Invasive",false);
	WurstplusSetting enderChest = create("Ender Chest","echest",true);
	WurstplusSetting closeAfter = create("Close After","CloseAfter",true);
	WurstplusSetting debugMode = create("Debug","Debug",false);
	...
	protected void enable() {
		...
		if (chatMsg.get_value(true))
			WurstplusMessageUtil.send_client_message("AutoSort Turned On!");
		...
		// Print the config
		if (chatMsg.get_value(true))
			WurstplusMessageUtil.send_client_message("Config " + curConfigName + " activated");
		// Get the inventory
	...
	private void sortInventoryAlgo() {
		if (!openedBefore) {
			...
			if (chatMsg.get_value(true)&& !doneBefore)
				WurstplusMessageUtil.send_client_message("Start sorting inventory...");
			...
			if (sortItems.size() == 0 && !doneBefore) {
				...
				// Print
				if (chatMsg.get_value(true))
					WurstplusMessageUtil.send_client_error_message("Inventory already sorted...");
			...
			if (stepNow == sortItems.size()) {
				...
				// Print
				if (chatMsg.get_value(true)) {
					WurstplusMessageUtil.send_client_message("Inventory sorted");
				}
	...
	private ArrayList<Integer> getInventorySort(HashMap<Integer, String> copyInventory,
		...
		if (debugMode.get_value(true)) {
			...
			for(int valuePath : planMove) {
				WurstplusMessageUtil.send_client_message(Integer.toString(valuePath));
			}
		}
