...
public class AutoGearCommand extends WurstplusCommand {
	...
	public boolean get_message(String[] message) {
		...
		if (msg.equalsIgnoreCase("save") || msg.equals("add") || msg.equals("create")) {
			if (message.length == 3) {
				...
				WurstplusMessageUtil.send_client_error_message("Not enough parameters!");
			}
...
//			}	else WurstplusMessageUtil.send_client_error_message("Not enough parameters!");
			...
			WurstplusMessageUtil.send_client_error_message("AutoGear message is: gear set/save/del/list [name]");
		...
		try {
			...
			for(int i = 0; i < lenghtJson; i++) {
				...
				if (!item.equals("pointer"))
					WurstplusMessageUtil.send_client_message("Kit available: " + item);
			}

		} catch (IOException e) {
			// Case not found, reset
			WurstplusMessageUtil.send_client_error_message("Kit not found!");
		}
	}
	...
	private void delete(String name) {
		...
		try {
			...
			}else WurstplusMessageUtil.send_client_error_message("Kit not found!");
	...
	private void save(String name) {
		...
		try {
			...
			completeJson = new JsonParser().parse(new FileReader(pathSave)).getAsJsonObject();
			if (completeJson.get(name) != null && !name.equals("pointer")) {
				WurstplusMessageUtil.send_client_error_message("This kit already exists!");
				return;
			}
			// We can continue
	...
	private void saveFile(JsonObject completeJson, String name, String operation) {
		...
		try {
			...
			bw.close();
			// Chat message
			WurstplusMessageUtil.send_client_message("Kit " + name + " " + operation);
		} catch (IOException e) {
			WurstplusMessageUtil.send_client_error_message("Error saving the file!");
		}
	}
