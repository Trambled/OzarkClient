...
public class WurstplusArrayList extends WurstplusPinnable {
	...
	public void render() {
		...
		if (Wurstplus.get_setting_manager().get_setting_with_tag("HUDEditor", "HUDArrayList").in("Top R") || Wurstplus.get_setting_manager().get_setting_with_tag("HUDEditor", "HUDArrayList").in("Top L") ) {
		...
		for (WurstplusHack modules : pretty_modules) {
			...
			if (flag) {
				...
				if (Wurstplus.get_setting_manager().get_setting_with_tag("HUDEditor", "HUDArrayList").in("Free")) {
					...
					if (Wurstplus.get_setting_manager().get_setting_with_tag("HUDEditor", "HUDArrayList").in("Top R")) {
					...
					if (Wurstplus.get_setting_manager().get_setting_with_tag("HUDEditor", "HUDArrayList").in("Top L")) {
						mc.fontRenderer.drawStringWithShadow(module_name, 2, 3 + count * 10, new WurstplusDraw.TravisColor(nl_r,nl_g,nl_b,nl_a).hex());
						count++;
					}
					if (Wurstplus.get_setting_manager().get_setting_with_tag("HUDEditor", "HUDArrayList").in("Bottom R")) {
					...
					if (Wurstplus.get_setting_manager().get_setting_with_tag("HUDEditor", "HUDArrayList").in("Bottom L")) {
